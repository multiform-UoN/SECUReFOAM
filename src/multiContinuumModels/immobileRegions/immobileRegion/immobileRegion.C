...
:
    ...
    rescaleBetas_(dict.lookupOrDefault<Switch>("rescaleBetas",false)),
    advDiff_(dict.lookupOrDefault<Switch>("advectionDiffusion",false)),
    Uimm_(NULL),
    Dimm_(NULL),
    ...
    betaCoeffs_(nOfTerms_)
{
    ...
    //- Read fields for advection-diffusion if required
    if(advDiff_)
    {
        Uimm_.set
        (
...
)
{
    ...
    if(!advDiff_)  solverPerformance::debug=0;
    ...
    if(!advDiff_) solverPerformance::debug=1;
...
)
{
    ...
    {       
        psiIEqn += 
            fvm::div(fvc::flux(Uimm_),psiImmobile,"div(phiI,psiI)")
          - fvm::Sp(fvc::div(Uimm_()),psiImmobile)
          - fvm::laplacian(Dimm_(),psiImmobile,"laplacian(DI,psiI)");
    }
